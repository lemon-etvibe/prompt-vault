...
[![Version](https://img.shields.io/badge/version-1.3.5-blue.svg)](https://github.com/lemon-etvibe/prompt-vault)
...
프롬프트 엔지니어링과 바이브 코딩을 하다 보면 시행착오는 피할 수 없습니다. 하지만 대부분의 개발자들은 컨텍스트가 압축되는 순간, 그 과정에서 쌓인 히스토리와 교훈을 모두 잃어버립니다.

**prompt-vault는 모든 AI 세션을 개인 회고록으로 만들어줍니다**: 시도한 것들을 자동으로 기록하고, 무엇이 효과적이었는지 리뷰하고, 페이즈 하나하나를 통해 AI 협업 스킬을 성장시키세요.

### 활용 예시

**🔍 바이브 코딩 성장을 위한 회고 분석**
복잡한 프롬프트 엔지니어링을 시도할 때 시행착오는 필연적입니다. Prompt Vault는 전체 인터랙션 히스토리를 기록해, 과거 프롬프트를 돌아보고 어떤 지시가 효과적이었는지 분석하며 AI 커뮤니케이션 스킬을 지속적으로 개선할 수 있게 해줍니다.

**🔄 장시간 AI 세션에서의 컨텍스트 복구**
복잡한 웹 개발이나 기획 작업 중 AI 컨텍스트 창이 꽉 차는 경우가 많습니다. Prompt Vault는 핵심 컨텍스트를 자동으로 저장하고 압축 후에도 끊김 없이 복구해, 이전 지시를 반복하는 번거로움 없이 연속적인 워크플로우를 유지할 수 있습니다.

**📋 페이즈 기반 문서화 & 팀 공유**
프로젝트를 단계별로 나눌 때(예: 기획 → DB 모델링 → UI 생성), 각 페이즈 히스토리를 깔끔한 HTML 리포트로 저장할 수 있습니다. AI의 추론 과정을 문서화하고 진행 상황을 리뷰하거나 팀원과 공유하기가 매우 쉬워집니다.
...
### Q8. `/clear` 시 자동 로깅이 동작하나요?

**A**: 아닙니다. `/clear`는 Claude Code 내장 커맨드로, 훅 시스템을 거치지 않고 직접 실행됩니다. pre-clear 훅 이벤트가 존재하지 않습니다. **`/clear` 전에 `/prompt-vault:log`를 실행하여** 현재 작업을 저장하세요. 자동 로깅이 활성화되어 있다면 마지막 응답 시점에 Stop 훅이 이미 저장했을 수 있지만, 그 이후 작업은 손실될 수 있습니다.
...
- **아키텍처 이해**: [ARCHITECTURE.ko.md](ARCHITECTURE.ko.md)에서 플러그인 내부 구조를 확인하세요
...
- **[ARCHITECTURE.ko.md](ARCHITECTURE.ko.md)** — 플러그인 기술 내부 구조 및 개발자 가이드
- **[GETTING_STARTED.ko.md](GETTING_STARTED.ko.md)** — 단계별 튜토리얼 및 실습 가이드
